...
class Manufacturing_energy:
    ...
    def __init__(self, year, energy_ghgrp):
        ...
        self.file_dir = '../data_foundation/data for calculations/'
        ...
        self.eia_epa_ids = pd.read_csv(
                'c:/Users/cmcmilla/Industry-energy-data-book/data_foundation/data for calculations/eia_epa_xwalk.csv',
                encoding='latin_1')
    ...
    def GHGRP_electricity_calc(self):
        ...
        base_file_dis = 'EIA923_Schedules_6_7_NU_SourceNDisposition_'
        
        base_file_gen = 'EIA923_Schedules_2_3_4_5_M_12_'
        ...
        with zipfile.ZipFile(io.BytesIO(r.content)) as data_zip:
            ...
            for f in file_list:
                ...
                if re.match(base_file_dis, f):
                    
                    with data_zip.open(f) as data_file:
                
                        eia923_dis = pd.read_excel(
                                data_file, skiprows=4,
                                sheet_name='Source_and_disposition'
                                )
                        ...
                        eia923_dis = pd.DataFrame(
                                eia923_dis[eia923_dis['Sector Code'].isin(
                                        [6, 7]
                                        )]
                                )
                ...
                if re.match(base_file_gen, f):
                    
                    with data_zip.open(f) as data_file:
                    
                        eia923_gen = pd.read_excel(
                                data_file, skiprows=5,
        ...
        for col in ['Incoming  Electricity', 'RE_gen', 'Retail Sales',
...
#                )
        ...
        county_electricity_energy_dd = dd.from_pandas(
                energy_nonghgrp.set_index('MECS_NAICS'),
                npartitions=len(self.mecs_naics.MECS_NAICS.unique())
                )
            
        county_electricity_energy_dd['naics'] =\
                county_electricity_energy_dd.naics.astype('int').compute()

        return county_electricity_energy_dd
